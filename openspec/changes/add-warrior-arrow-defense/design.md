...
- Make frontal blocks and rear damage deterministic from the Warrior's existing facing.
...
- Vertical-facing Warrior artwork or guarding against vertical arrows.

## Decisions

### Centralize defense timing and use existing facing

Define a frozen default defense configuration near the Warrior behavior contract with `reactionLookaheadSeconds` and `defenseDurationSeconds`. Start with a 0.40-second trajectory lookahead and the required 0.25-second defense duration. Validate/clamp configuration at Warrior creation. Compare the incoming arrow's horizontal travel direction with the Warrior's current horizontal facing: an arrow traveling opposite the facing direction comes from the front and always blocks; an arrow traveling with the facing direction comes from behind and never blocks. Vertical arrows do not trigger automatic guard because the Warrior has no vertical facing artwork.

This makes the counter entirely positional and frame-rate independent. Turning toward an incoming arrow was rejected because it would erase the intended rear vulnerability; retaining a probability roll was rejected because frontal and rear outcomes must be guaranteed.
...
Add an automatic-defense timer and read-only defending state to the Warrior actor. A frontal reaction preserves the Warrior's facing, enters guard immediately, interrupts any locomotion or attack animation, and holds guard for 0.25 seconds of active gameplay time. When the timer expires, the actor returns to current idle/walking intent. Rear arrows do not alter facing or action state.
...
Before applying Warrior projectile damage, query the Warrior's active defense state. A defending hit calls projectile deflection and skips damage/knockback/hit flash. An undefended hit retains the existing 50 damage and normal removal. Sheep and goblin projectile behavior remains unchanged.
