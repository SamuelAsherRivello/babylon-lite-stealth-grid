...
SHALL ignore all other contact sources for damage. An arrow that hits a Warrior
during the Warrior's active defense pose SHALL deal no damage; an arrow that
hits a Warrior outside that pose SHALL reduce Warrior health by 50.
...
- **WHEN** a hero arrow collider overlaps a Warrior whose defense pose is not active
- **THEN** Warrior health is reduced by 50 and the normal arrow-hit lifecycle applies

#### Scenario: Hero shoots defending Warrior
- **WHEN** a hero arrow collider overlaps a Warrior whose defense pose is active
